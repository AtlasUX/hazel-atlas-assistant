...
module.exports = ({ cache, config }) => {
  ...
  exports.nupkg = async (req, res) => {
    
    // Get the latest version from the cache
    const { platforms } = await loadCache()
    
    const nuggetPackageAsset = platforms['nupkg']
    
    if (!nuggetPackageAsset) {
      send(res, 404, 'No download available!')
      return
    }
    
    // Assert that nuggetPackageAsset.name is the same as the name in the URL
    const { name } = req.params
    if (nuggetPackageAsset.name !== name) {
      send(res, 404, 'No download available!')
      return
    }

    proxyPrivateDownload(nuggetPackageAsset, req, res)
  }
